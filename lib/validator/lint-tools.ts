        ...
        tokens.forEach((tok) => {
          ...
          if (!requestRootsAllowed.has(root)) {
            issues.push({
              ...
              message: `Only args.* or secrets.* may be used in the request. Found ${tok}.`,
        ...
        tokens.forEach((tok) => {
          ...
          if (!uiRootsAllowed.has(root)) {
            issues.push({
              ...
              message: `UI may use args.*, response.*, or status. Found ${tok}.`,
    ...
    if (reqArgs.size > 0) {
      for (const a of argsUsedInRequest) {
        if (!reqArgs.has(a)) {
          issues.push({
            severity: "warning",
            code: "parameters.required_mismatch",
            path: "parameters.required",
            message: `Template references args.${a} but it is not listed in parameters.required.`,
            suggestion: `Add "${a}" to parameters.required or stop referencing it.`,
          });
        }
      }
    }
...
/* ----------------------------- Example usage -----------------------------
import { actions } from "@/lib/actions"; // your descriptors array

const results = lintHttpToolDescriptors(actions);

// Render in admin UI:
for (const r of results) {
  console.group(`Tool ${r.name} (${r.tenantId ?? "global"})`);
  for (const i of r.issues) {
    console[i.severity === "error" ? "error" : "warn"](
      `${i.severity.toUpperCase()} [${i.code}] @ ${i.path}: ${i.message}`,
