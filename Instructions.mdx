...
<StepCard title="Define Custom Tools" description="Build HTTP-based tools to connect the agent to your APIs and databases. These allow actions like querying inventory, creating bookings, or searching products.">
  ...
  2. **Define the Schema**: Each tool follows this structure (based on Zod schema for validation):
     ...
     {
       "kind": "http_tool",
       "name": "your_tool_name",  // Unique, alphanumeric with ._- 
       "description": "Brief tool purpose",
       "parameters": {  // JSON Schema for args the agent must collect
         "type": "object",
         "properties": {
           "param1": { "type": "string" },
           // Add required fields, enums, patterns, etc.
         },
         "required": ["param1"]
       },
       "http": {
         "method": "GET|POST|PUT|PATCH|DELETE",
         "urlTemplate": "https://your-api.com/endpoint/\{\{param1\}\}",  // Use \{\{ \}\} for interpolation
         "headers": { "authorization": "Bearer \{\{secrets.api_key\}\}" },  // Use secrets for auth
         "jsonBodyTemplate": { "key": "\{\{param1\}\}" },  // For POST/PUT bodies
         "okField": "ok",  // Field in response indicating success
         "timeoutMs": 8000,  // Max 120000
         "pruneEmpty": true  // Remove empty fields from request
       },
       "ui": {  // Optional: Auto-open visuals on success/error
         "onSuccess": {
           "open": {
             "component_name": "catalog_results",
             "title": "Results",
             "props": { "items": "\{\{response.items\}\}" }  // Interpolate response data
           }
         },
         "onError": { /* Similar structure */ }
       },
       "enabled": true,
       "priority": 5,  // Higher = preferred in conflicts
       "version": 1
     }
     ```
